<template>
  <div>
    <v-carousel hide-delimiters>
      <v-carousel-item
        v-for="(item, index) in this.sliderItems" :src="require('../assets/img/home/' + item.imgPath)" :key="index"
      >
        <v-row
          class="fill-height"
          align="center"
          justify="center"
        >
          <div class="display-2 white--text pl-5 pr-5 hidden-sm-only"><strong>{{ item.title }} </strong></div><br/>
        </v-row>
      </v-carousel-item>
        ...
        <v-card outlined>
          ...
          >
            ...
            <div class="text-center mt-2">
              <v-btn class="white--text caption" text @click="() => $router.push({ name: 'products', params: item.id })">SHOP NOW <v-icon class="white--text caption">mdi-arrow-right</v-icon></v-btn>
            </div>
          </v-img>
        </v-card>
      </div>
      ...
      <v-row no-gutters>
        <v-col :cols="12">
          ...
          >
            ...
            <v-divider></v-divider>
            <div class="row">
              <div class="col-12 col-md-3 col-sm-6 col-xs-6 text-center" v-for="item in this.latestProducts" :key="item.id">
                <v-hover
                ...
                >
                  <div class="gray-overlay">
                    <v-img
                      ...
                      height="250px"
                      :src="'http://localhost:5010/ProductImages/' + item.images[0]"
                    >
                    </v-img>

                  </div>

                  <v-card-title class="black--text pt-5">{{ item.category }} </v-card-title>
...
export default {
  ...
  beforeCreate: function() {
    this.$http.get("lookup/getlatestproducts")
      .then(response => {
